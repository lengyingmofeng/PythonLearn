    ...
    def _find_outer(self, k, words):
        ...
        for pos, text in words.items():
            if re.search(r'发票$', text):  # 发票名称
                ...
                reset = re.split("复核:", text)
                data = re.search("复核", text)
...
def csv_to_xml(csv_file, xml_file):
    """
    将 CSV 文件内容转换为 XML 文件

    参数:
    - csv_file: 输入的 CSV 文件路径
    - xml_file: 输出的 XML 文件路径
    """
    # 读取CSV文件
    ...
    for _, row in data.iterrows():
        ...
        ET.SubElement(header, "EIid").text = str(row['发票代码'])
        ET.SubElement(header, "EInvoiceTag").text = "SWEI4400"
        ET.SubElement(header, "Version").text = "0.34"
        # 创建发票元素
        invoice = ET.SubElement(root, "invoice")

        # 添加发票编号
        ET.SubElement(invoice, "invoice_number").text = str(row['发票代码'])

        # 添加开票日期
        ET.SubElement(invoice, "issue_date").text = str(row['开票日期'])

        # 添加买方信息
        buyer = ET.SubElement(invoice, "buyer")
        ET.SubElement(buyer, "buyer_name").text = row['购买方名称']
        ET.SubElement(buyer, "buyer_tax_id").text = str(row['购买方纳税人识别号'])

        # 添加卖方信息
        seller = ET.SubElement(invoice, "seller")
        ET.SubElement(seller, "seller_name").text = row['销售方名称']
        ET.SubElement(seller, "seller_tax_id").text = str(row['销售方纳税人识别号'])

        # 添加金额
        ET.SubElement(invoice, "amount").text = str(row['金额'])

    # 创建一个ElementTree对象并保存到XML文件
    tree = ET.ElementTree(root)
    tree.write(xml_file, encoding="UTF-8", xml_declaration=True)

    print(f"XML文件已生成：{xml_file}")


if __name__ == "__main__":
    path = r'I:/发票公司/1月044031800311_14158781.pdf'
    data = Extractor(path).extract()
    # 保存为 XML 文件
    data.to_csv('invoice_data.csv', index=False, encoding='utf-8')
    # 调用方法示例
    csv_to_xml('invoice_data.csv', 'invoice_data.xml')
